...
# In[]
# Create a ring class for stimulation of a ring network
class Ring:
...
# In[]
# Compare effects of synaptic weight
plt.figure()
...
# In[]
# Display positions of the cells
ring1 = Ring(N=6)
cells_positions = []
for cell in ring1.cells:
    cells_positions.append([cell.x, cell.y, cell.z])
cells_positions = np.array(cell_positions)
...
ax.scatter(cells_positions[:,0], cells_positions[:,1], cells_positions[:,2])
...
ring.cells[0]._ncs[0].syn()
ring.cells[1]._ncs[0].syn()
ring.cells[2]._ncs[0].syn()

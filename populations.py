...
class RandomPopulation():
    ...
    def add_stim_random_cells (self, N_cells=0.05, n=1, onset=5, delay=5, stim_w=0.0005):
        """
        N_cells     number of cells to choose randomly (uniform distribution).
        ...
        if N_cells < 1:
            N_cells = int(round(N_cells * self.N, 0))

        self._netstim = h.NetStim()
        self._netstim.number = n
        self._netstim.start = onset * ms

        stim_indices = np.arange(len(self.cells))
        stim_indices = np.random.choice(stim_indices, N_cells, replace=False)
    ...
    def add_current_random_cells (self, N_cells='all', duration=20, delay=5, amp=0.031):
        """
        N_cells     number of cells to choose randomly (uniform distribution).
                    May be int or a fraction of population N
        n           number of pulses sent to the cell
        ...
        for i in stim_indices:
            ...
            stim.dur = duration
            stim.amp = amp
            self.cells[i]._step_currents.append(stim)

...
class RandomPopulation():
    ...
    def connect_to(self, other_population, N_input=0.2, N_output=0.2, excitatory=None, syn_w=0.0005, syn_delay=5):
        ...
        input_indices = np.random.choice(input_indices, N_input)
        for i in input_indices:
            source = self.cells[i]
            outputs = np.random.choice(other_population.cells, N_output) # choose output cells
            for target in outputs:
                ...
                return (i,target._gid)

# In[]
# __________________Test excitatory connection
exct_1 = RandomPopulation(N=5, N_interconnect=1)
exct_2 = RandomPopulation(N=5, N_interconnect=1)

ind = exct_1.connect_to(exct_2, N_input=1, N_output=1, syn_w=0.0005)


# In[]
# exct_1.cells[0].soma.psection()
# exct_1.cells[0]._ncs
# exct_1.cells[1]._ncs
# exct_1.cells[2]._ncs
# exct_1.cells[3]._ncs
...
# exct_2.cells[3]._ncs
# exct_2.cells[4]._ncs
# exct_2.cells[2].soma.psection()

# In[]
# Stimulate the cell

# excitatory.cells[0]._ncs[0].syn()
stim = h.NetStim()

syn_ = h.ExpSyn(exct_1.cells[ind[0]].soma(0.5))

stim.number = 1
stim.start = 9 * ms
ncstim = h.NetCon(stim, syn_)
ncstim.delay = 5 * ms
ncstim.weight[0] = 0.0005 # NetCon weight is a vector.
...
# In[]
plt.figure("V_m")
plt.plot(t, exct_1.cells[ind[0]].soma_v, ':r')
plt.plot(t, exct_2.cells[ind[1]].soma_v, '-b')
...
# __________________Test inhibitory connection
inh = RandomPopulation(N=5, N_interconnect=1, excitatory=False)
exct = RandomPopulation(N=5, N_interconnect=1)

ind = inh.connect_to(exct, N_input=1, N_output=1, syn_w=0.0005)

# In[]
# Stimulate the cell
stim = h.NetStim()

syn_ = h.ExpSyn(inh.cells[ind[0]].soma(0.5))
...
# In[]
plt.figure("V_m")
plt.plot(t, inh.cells[ind[0]].soma_v, ':r')
plt.plot(t, exct.cells[ind[1]].soma_v, '-b')
plt.title('Inhibitory synapse from pone population to another')
plt.xlabel('t (ms)')
plt.ylabel('V$_m$ (mV)')
plt.show()
